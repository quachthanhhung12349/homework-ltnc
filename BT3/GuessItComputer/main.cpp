...
int main(int argc, char* argv[])
{
    ...
    srand(time(0));
    displayStartGame();
    int randomNumber = getPlayerGuess();


    int l = 1, r = 100, number, answer;
    do {
        number = (l+r)/2;
        displayPlayerGuess(number);
        updateAnswer(answer, markedNumbers, number, randomNumber);
        displayAnswer(answer, number);
        if (answer == TOO_BIG) r = number-1;
        if (answer == TOO_SMALL) l = number+1;
...
void printStartMsg()
{
    renderTexture(startMsg, renderer, 296, 95);
    SDL_RenderPresent(renderer);
}

void printComputerMsg()
{
    renderTexture(computerMsg, renderer, 294, 100);
    SDL_RenderPresent(renderer);
}
...
void printComputerComparision(SDL_Texture *tex)
{
    renderTexture(tex, renderer, 294, 115);
    SDL_RenderPresent(renderer);
}

void printHumanMsg()
{
    renderTexture(humanMsg, renderer, 555, 130);
    SDL_RenderPresent(renderer);
}

void printHumanNumber(SDL_Texture *tex)
{
    renderTexture(tex, renderer, 570, 132);
    SDL_RenderPresent(renderer);
}
